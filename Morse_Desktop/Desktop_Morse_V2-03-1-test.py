...
############################################################################################################
 ...
 Filename     : Morse_desktop_v2-03-1-test.py
 ...
 version      : V 2.03-1-test
 
 -->  -->  -->  TEST VERSION FOR THE TIMER  <--  <--  <--
 ...
 * V 2.03-1-test :
 - Test version to resolve the timer's problem  with time's adding in the loops or the 'do it again' function jump.
...
Future ehancements :  '-' = to be done    '/' = in developpement    'X' = finished
 ...
 / Showing the elapsed time for coding in Morse
    ...
    while True:                                             # While True loop for the menu choice                       
        try:                                                # Exception try for the choice menu
            ...
            if menu_ch == 1:
                ...
                code_in_morse()                                  # Execute code_ltr() function 
            elif menu_ch == 2:
...
def sec2ydhms(ss):                                           # Conversion function returns an amount of seconds in years, days, hours, minutes and seconds
                                                             # using the 'divmod' --> (quotient, remainder) = (value 1 / value 2)
    (yy, ss) = divmod(ss, 31536000)                          # (yy 'years', ss 'seconds') = divmod(ss (amount of seconds / 31536000 (amount of seconds in a year))                         
    (dd, ss) = divmod(ss, 86400)                             # 86400 --> amount of seconds in a day
    (hh, ss) = divmod(ss, 3600)                              # 3600 --> amount of seconds in an hour
    (mm, ss) = divmod(ss, 60)                                # 60 amount of seconds in a minute
    return (yy, dd, hh, mm, ss)                              # Returns the amounts of years, days, hours, minutes and seconds
                                                             # --> only minutes and seconds are used in this program


def code_in_morse():                                         # Function to code in Morse a letter, a word, a sentence or some numbers
    tmr = 0
    while True:
        try:
        ...
        tmr = round(tmr)
        (years, days, hours, minutes, seconds) = sec2ydhms(tmr)    # Calls the 'sec2ydhms to convert the timer 'tmr' (tmr is a global variable)
